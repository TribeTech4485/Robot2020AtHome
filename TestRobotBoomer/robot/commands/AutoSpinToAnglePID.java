...
public class AutoSpinToAnglePID extends CommandBase {
  ...
  public void execute() {
    ...
    if (pidOutput >= -0.3 && pidOutput <= 0.3) {
        speed = turnPower * 0.80;   // slow down if close to target
    } else {
        speed = turnPower;          // else do speed requested 
    }
    if (targetAngle > 0) {

      if (counter1++ % 2 == 0) { System.out.println("**turn Left Correction pidOut: "+String.format("%.3f  ", pidOutput)+" heading: "+String.format("%.3f", currentHeading)+" angle "+targetAngle); }
      m_driveTrain.doTankDrive(-speed, speed); // turn to left ( reverse left side)
    } else {

        if (counter2++ % 2 == 0) { System.out.println("**turn Right Correction pidOut: "+String.format("%.3f  ", pidOutput)+" heading: "+String.format("%.3f", currentHeading)+" angle "+targetAngle); }
